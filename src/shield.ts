...
function normalizeOptions(options: IOptionsConstructor): IOptions {
  ...
  return {
    ...
    fallbackError: withDefault<IFallbackErrorType>(new Error('Not Authorised!'))(
...
export function shield<TSource = any, TContext = any, TArgs = any>(
  ...
  if (ruleTreeValidity.status === 'ok') {
    const generatorFunction = generateMiddlewareGeneratorFromRuleTree<TSource, TContext, TArgs>(
      ruleTree,
      normalizedOptions,
    );
